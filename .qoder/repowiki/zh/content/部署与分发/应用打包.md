...
## 更新摘要
**变更内容**
- 更新了路径解析系统部分，反映新的资源路径解析机制
- 新增了开发环境与PyInstaller环境的路径解析对比
- 完善了资源文件打包策略的技术细节
- 增强了跨平台兼容性的说明
...
- 资源路径工具：在冻结环境下通过 PyInstaller 注入的运行时路径访问资源；在开发环境下通过相对路径解析。
...
### 资源路径解析与跨平台兼容
- **改进的路径解析系统**：资源路径工具在冻结环境下通过 PyInstaller 注入的运行时路径访问资源；在开发环境下通过相对路径解析到项目根目录。
- **双环境支持机制**：
  - 冻结环境：使用 `sys._MEIPASS` 获取临时解压目录
  - 开发环境：从 `utils` 目录向上遍历到 `controller` 目录
- 主窗口与进度条组件均通过该工具加载图片资源，确保打包后仍能正确解析路径。

```mermaid
flowchart TD
Start(["进入 resource_path"]) --> CheckFrozen{"是否处于冻结环境？"}
CheckFrozen --> |是| UseMeipass["使用 sys._MEIPASS 临时目录"]
CheckFrozen --> |否| UseDev["使用相对路径解析到项目根目录"]
UseMeipass --> JoinPath["拼接资源路径并返回"]
UseDev --> ResolveBase["从 utils 目录向上遍历到 controller"]
ResolveBase --> JoinPath
...
- **资源访问优化**
  ...
  - 利用改进的路径解析系统，避免重复的路径计算。

## 故障排查指南
- **资源路径问题**
  - 症状：运行时报找不到资源文件。
  - 排查：确认资源路径工具在冻结环境与开发环境的行为一致；检查资源是否被打包进应用。
  - **新增**：验证 `sys._MEIPASS` 是否正确设置，以及相对路径解析是否能到达正确的项目根目录。
- **PySide6 相关问题**
  - 症状：界面空白或控件不显示。
...
## 结论
本项目基于 PySide6 的 Qt 应用，通过统一的资源路径解析实现跨平台资源访问。改进的路径解析系统确保了在开发环境和 PyInstaller 打包环境中的资源正确加载。结合 PyInstaller 的隐藏导入与资源打包策略，可在多平台生成可执行文件。建议在实际打包时：
- 明确隐藏导入与资源打包范围；
- 针对不同平台调整构建参数；
- 采用体积与启动时间优化手段；
- 建立完善的故障排查流程。
...
- **资源打包策略**
  - 使用改进的资源路径工具统一访问资源，确保冻结环境可用。
  - 对于图片等静态资源，建议在打包前进行尺寸与格式优化。
  - **新增**：确保 `assets` 目录结构在打包前后保持一致。
- **平台差异**
...
- **打包优化**
  ...
  - **新增**：利用改进的路径解析系统，减少路径解析开销。
- **常见错误与解决**
  - 资源缺失：检查资源路径与打包包含规则。
  - 模块导入失败：补充隐藏导入；确认第三方库版本兼容。
  - 平台特定问题：针对平台差异调整构建参数与权限。
  - **新增**：路径解析失败：检查 `sys._MEIPASS` 环境变量和相对路径解析逻辑。
